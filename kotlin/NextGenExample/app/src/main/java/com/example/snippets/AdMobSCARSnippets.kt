...
class AdMobSCARSnippets {
  ...
  fun loadNative(adUnitID: String) {
    ...
    val signalRequest: NativeSignalRequest =
      NativeSignalRequest.Builder(signalType = "SIGNAL_TYPE")
    ...
    MobileAds.generateSignal(
      ...
      object : SignalGenerationCallback {
        override fun onSuccess(signal: Signal) {
          ...
          val adResponseString = fetchSignalResponse(signal)
        }
  ...
  fun loadBanner(applicationContext: Context, adUnitID: String) {
    ...
    val signalRequest: BannerSignalRequest =
      BannerSignalRequest.Builder(signalType = "SIGNAL_TYPE")
        .setAdUnitId(adUnitID)
        .setAdSize(size)
        .build()
    ...
    MobileAds.generateSignal(
      ...
      object : SignalGenerationCallback {
        ...
        override fun onFailure(error: SignalError) {
          Log.d(TAG, "Error generating signal: ${error.message}")
        }
      },
    )
  ...
  fun loadNativeWithOptions(adUnitID: String) {
  ...
  private fun fetchSignalResponse(signal: Signal): String {
    ...
    NativeAdLoader.loadFromAdResponse(
      ...
      object : NativeAdLoaderCallback {
        ...
        override fun onAdFailedToLoad(adError: LoadAdError) {
          Log.d(TAG, "Error rendering native ad: ${adError.message}")
        }
      },
    )
    // [END render_native]
  ...
  fun renderBanner(activity: Activity, adResponseString: String) {
    ...
    adView.loadFromAdResponse(
      ...
      object : AdLoadCallback<BannerAd> {
        ...
        override fun onAdFailedToLoad(adError: LoadAdError) {
          Log.d(TAG, "Error rendering banner ad: ${adError.message}")
        }
      },
    )
    // [END render_banner]
